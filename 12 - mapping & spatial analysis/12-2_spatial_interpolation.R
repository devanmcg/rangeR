...
# Combine stations with map
  ...
  (gg2 <- ggplot()  + coord_map("polyconic") + theme_bw() + 
...
# Look at trends over time 
  ...
  # Get interpolations for each month w/ custom function 
  # (see for loop and dplyr alternatives below)
    monthly.interps <- ddply(fall.monthly, 
                             ...
                             function(x) {
                               ...
                               return(interp.df) } ) 
  # Plot
    ggplot()  + coord_map("polyconic") + theme_bw() +
...
# An alternative to ddply using a for loop
    ...
    for(i in 1:length(unique(fall.monthly$Month))) {
      M = unique(fall.monthly$Month)[i]
...
# A (better?) alternative using dplyr (best for piping)
    # Here one does want to define the function:
